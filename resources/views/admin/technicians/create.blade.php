    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-black text-2xl uppercase tracking-tighter text-[#1A1A1A]">
                Tambah <span class="text-[#D92323]">Teknisi</span>
            </h2>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white border-4 border-[#1A1A1A] shadow-[8px_8px_0px_#1A1A1A] rounded-3xl p-8">
                <div class="flex items-center gap-3 mb-6 border-b-4 border-gray-100 pb-4">
                    <div class="w-12 h-12 bg-[#22C55E] border-4 border-[#1A1A1A] rounded-full flex items-center justify-center font-black text-[#1A1A1A] text-xl">
                        ➕
                    </div>
                    <div>
                        <h3 class="font-black text-xl uppercase italic text-[#1A1A1A]">Data Personel Baru</h3>
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Buat akun untuk teknisi lapangan</p>
                    </div>
                </div>

                <form action="{{ route('admin.technicians.store') }}" method="POST" class="space-y-6">
                    @csrf
                    
                    <div>
                        <label class="block text-[11px] font-black uppercase text-[#1A1A1A] mb-2 tracking-widest">Nama Lengkap</label>
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Ahsani Taqwim" required 
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="teknisi@widi.com" required 
                            class="w-full border-4 border-[#1A1A1A] rounded-xl p-4 text-sm font-bold focus:ring-0 focus:border-[#D92323] transition-all bg-gray-50 focus:bg-white">
                    ...
                    <div>
                        <label class="block text-[11px] font-black uppercase text-[#1A1A1A] mb-2 tracking-widest">Password</label>
